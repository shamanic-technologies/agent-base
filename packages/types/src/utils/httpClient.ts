 ...
 * @param {string} [platformUserId] - Optional platform user ID to propagate in the 'x-platform-user-id' header.
...
export async function makeServiceRequest<T>(
  ...
  try {
    ...
    if (typeof response.data === 'object' && response.data !== null && 'success' in response.data) {
       console.log(`[httpClient] Received successful response from ${fullUrl}`);
