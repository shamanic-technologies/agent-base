 ...
 */
export async function getPlatformUserFromDatabase(platformUserId: string, platformOrganizationId: string): Promise<ServiceResponse<PlatformUser>> {

  // Use the API client function
  // The client function handles passing the platformUserId for the header
  const response = await getCurrentPlatformUser(platformUserId, platformOrganizationId);
